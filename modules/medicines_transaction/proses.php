...
if (empty($_SESSION['username']) && empty($_SESSION['password'])){
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['Guardar'])) {
            ...
            $comentario = mysqli_real_escape_string($mysqli, trim($_POST['comentario']));
            $tipo_transaccion = mysqli_real_escape_string($mysqli, trim($_POST['transaccion']));
            $created_user    = $_SESSION['id_user'];

            $query = null;

            if(strlen(trim($_POST['paciente'])) > 0 && strlen(trim($_POST['comentario'])) > 0 ){
                $id_paciente = mysqli_real_escape_string($mysqli, trim($_POST['paciente']));
                $comentario = mysqli_real_escape_string($mysqli, trim($_POST['comentario']));

                $query = mysqli_query($mysqli, "INSERT INTO transaccion_medicamentos(codigo_transaccion,fecha,codigo,numero,created_user,tipo_transaccion,id_paciente,comentario) 
                                            VALUES('$codigo_transaccion','$fecha_a','$codigo','$num','$created_user','$tipo_transaccion', '$id_paciente','$comentario')")
