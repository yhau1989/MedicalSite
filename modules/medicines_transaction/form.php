...
<script type="text/javascript">
  ...
  function hitung_total_stok() {
    ...
    document.getElementById("form-group-paciente").style.display = "block";
    document.getElementById("form-group-comentario").style.display = "block";
    document.getElementById("s_paciente").required = true;
    document.getElementById("i_comentario").required = true;
	}	else {
		document.formObatMasuk.total_stok.value = (hasil);
    document.getElementById("form-group-paciente").style.display = "none";
    document.getElementById("form-group-comentario").style.display = "none";
    document.getElementById("s_paciente").required = false;
    document.getElementById("i_comentario").required = false;
	} 
          ...
          <form role="form" class="form-horizontal" action="modules/medicines_transaction/proses.php?act=insert" method="POST" name="formObatMasuk">
            <div class="box-body">
              ...
              <div class="form-group">
               ...
               <div id="form-group-paciente" class="form-group">  
                <label class="col-sm-2 control-label">Paciente</label>
                <div class="col-sm-5">
                  <select id="s_paciente" class="chosen-select" name="paciente" data-placeholder="-- Seleccionar Paciente --" onchange="tampil_obat(this)" autocomplete="off">
                    <option value=""></option>
                    <?php
                      $query_obat = mysqli_query($mysqli, "SELECT id, nombres, apellidos FROM pacientes ORDER BY nombres ASC")
                                                            or die('error '.mysqli_error($mysqli));
                      while ($data_obat = mysqli_fetch_assoc($query_obat)) {
                        echo"<option value=\"$data_obat[id]\"> $data_obat[nombres]  $data_obat[apellidos] </option>";
                      }
                    ?>
                  </select>
                </div>
              ...
              <div id="form-group-comentario" class="form-group">
                ...
                <div class="col-sm-5">
                  <input id="i_comentario" type="text" class="form-control" name="comentario" size="200">
                </div>
              </div>
